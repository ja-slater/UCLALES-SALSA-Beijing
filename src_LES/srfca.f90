...
MODULE srfc
 ...
 REAL	   :: C_heat1 = 9.e6
   ...
   REAL ::  fii_s1 = -0.036
   REAL ::  fii_s3 = -0.085
   REAL ::  thetaS1 = 0.109 ! Soil porosity
   REAL ::  thetaS2 = 0.109
   REAL ::  thetaS3 = 0.0488  
...
CONTAINS
   ...
   SUBROUTINE surface()
      ...
      USE grid, ONLY: nzp, nxp, nyp, a_up, a_vp, a_theta, a_rv, a_rp, zt, dzt, psrf, th00,  &
                      ...
                      W1,W2,W3, mc_ApVdom, dtlt,sst,xx,sst1,sst2,sst3,sst4
      USE thrm, ONLY: rslf
      ...
      INTEGER :: i, j, iterate, ii, jj, mm, n 
      REAL    :: zs, bflx, ffact, bflx1, Vbulk, Vzt, usum, x, zs1,zs2,zs3,zs4
      REAL (kind=8) :: bfl(2), bfg(2)

      REAL :: mctmp(nxp,nyp) ! Helper for mass concenrvation statistics
	
	!!real,allocatable,dimension(:) :: sinrange
	...
	lambda1=1.5e-7*C_heat1
	lambda2=1.5e7*C_heat2
	lambda3=1.5e7*C_heat3
	lambda4=1.5e7*C_heat4


        ! Determine moisture at different depths in the ground
	!CALL qflux(sinrange,qflx,xx,qflxstep,i,n,n3,n2)
        ...
        !
            ...
            DO j = 3, nyp-2
               DO i = 3, 28
                  ...
                  !drdz(i,j) = a_rp(2,i,j) - ff1*rslf(psrf,sst1)  !  a_rv changed to a_rp (by Zubair)
                  !
                  bfct(i,j) = g*zt(2)/(a_theta(2,i,j)*wspd(i,j)**2)
                  usum = usum + a_ustar(i,j)
               END DO
	...
	END DO
		END DO
		 DO j = 3, nyp-2
               DO i = 29, 54
!
                  dtdz(i,j) = a_theta(2,i,j) - sst2*(p00/psrf)**rcp
!
                 ff1 = 1.0
                  IF(W1 < 0.75) ff1 = W1/0.75
                !  ! Flux of moisture is limited by water content.
                  !drdz(i,j) = a_rp(2,i,j) - ff1*rslf(psrf,sst2)  !  a_rv changed to a_rp (by Zubair)
              !    !
              bfct(i,j) = g*zt(2)/(a_theta(2,i,j)*wspd(i,j)**2)
               usum = usum + a_ustar(i,j)
             END DO
	 ...
	 DO j = 3, nyp-2
          DO i = 55,80
        dtdz(i,j) = a_theta(2,i,j) - sst3*(p00/psrf)**rcp

         ff1 = 1.0
          IF(W1 < 0.75) ff1 = W1/0.75
        ! Flux of moisture is limited by water content.
          !drdz(i,j) = a_rp(2,i,j) - ff1*rslf(psrf,sst3)  !  a_rv changed to a_rp (by Zubair)
         !    !
             bfct(i,j) = g*zt(2)/(a_theta(2,i,j)*wspd(i,j)**2)
              usum = usum + a_ustar(i,j)
            END DO
 	...
 	DO j = 3, nyp-2
               DO i = 81, 106
                 ...
                 dtdz(i,j) = a_theta(2,i,j) - sst4*(p00/psrf)**rcp

                  ff1 = 1.0
                  IF(W1 < 0.75) ff1 = W1/0.75
                  ! Flux of moisture is limited by water content.
                 !drdz(i,j) = a_rp(2,i,j) - ff1*rslf(psrf,sst4)  !  a_rv changed to a_rp (by Zubair)
                  !
                  bfct(i,j) = g*zt(2)/(a_theta(2,i,j)*wspd(i,j)**2)
                  usum = usum + a_ustar(i,j)
               END DO
 ...
 sst1 = sst1-(total_rw-75+total_la+total_se+( SQRT(lambda1*C_heat1*7.27e-5/(2.0)) *(SST1-deepSoilTemp)))&
                /(2.0e-2*C_heat1+ SQRT( lambda1*C_heat1/(2.0*7.27e-5)) )*dtl

	
           sst2 = sst2-(total_rw-75+total_la+total_se+( SQRT(lambda2*C_heat2*7.27e-5/(2.0)) *(SST2-deepSoilTemp)))&
                /(2.0e-2*C_heat2+ SQRT( lambda2*C_heat2/(2.0*7.27e-5)) )*dtl
	
	!	print *, sst, sst2



        sst3 = sst3-(total_rw-75+total_la+total_se+( SQRT(lambda3*C_heat3*7.27e-5/(2.0)) *(SST3-deepSoilTemp)))&
                /(2.0e-2*C_heat3+ SQRT( lambda3*C_heat3/(2.0*7.27e-5)) )*dtl
           ...
           sst4 = sst4-(total_rw-75+total_la+total_se+(SQRT(lambda4*C_heat4*7.27e-5/(2.0)) *(SST4-deepSoilTemp)))&
                /(2.0e-2*C_heat4+ SQRT( lambda4*C_heat4/(2.0*7.27e-5)) )*dtl
	    ...
	    !CALL qflux(qflx,nxp,nyp)
	         
		!print *, sst, sst1, sst2, sst3, sst4
...
CASE DEFAULT
   ...
   SUBROUTINE srfcscls(n2,n3,z,z0,th00,u,dth,drt,ustar,tstar,rstar)
      ...
      REAL    :: x, y, psi1, psi2, zeta, lmo, dtv,lnz1,lnz2,lnz3,lnz4
     ! REAL    :: klnz1, klnz2, klnz3, klnz4
...
DO j = 3, n3-2
  ...
  ! SUBROUTINE qflux(qflx,n3,n2)
	 
        !USE STEP, ONLY : timmax
	!USE GRID, ONLY : dtl,xx
	!IMPLICIT NONE

	! INTEGER, intent(in) :: n2,n3
         !REAL, intent(out)   :: qflx(n2)

         !REAL :: x(n2,n3),y(n2,n3)
     	 !INTEGER :: ii,j

	!sinrange=(/(xx+qflxstep, i=1,n)/)
	!real,allocatable,dimension(:) :: sinrange
	!real,parameter :: pi=3.1415926535 
