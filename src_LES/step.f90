...
CONTAINS
   ...
   SUBROUTINE stepper
      ...
      REAL(kind=8) :: cflmax,gcflmax,time_prev
      INTEGER      :: istp, iret
      LOGICAL :: cflflg

	!REAL :: dummy
	!open(45,file='cd_surf_temp',status='old',form='formatted') !! Added by JS to try and force surftemp
	!	!!! NO idea how this works but giving it a try.
      ...
      DO WHILE (time + 0.1*dtl < timmax)
         ...
         time = time + dtl
   !!! ADDED BY JS
		!IF(time-time_prev>0.9999) then 
	!read(45,*) dummy, T_s, ntime
	!time_prev=floor(time)+1.e-8
        !END IF
!!!READ in cd_surf_temp
   ...
   SUBROUTINE t_step(cflflg,cflmax)
      ...
      CALL surface() !!EDITED BY JS from surface() to surface(sst,time,ntime,T_s)
